      ...
      <div className="glass-panel rounded-2xl p-4 pointer-events-auto flex flex-col gap-4 overflow-y-auto max-h-full">
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-semibold text-slate-300 flex items-center justify-between">
            ...
            <span className="text-[10px] text-slate-500 font-normal">即時生成 3D 樹</span>
          </label>
          <div className="relative">
            <input
              ...
              className="w-full glass-input rounded-xl px-3.5 py-2 text-xs text-white placeholder-slate-500 outline-none transition focus:border-pink-500"
            />
          </div>
        </div>
        ...
        {isExpanded && (
          <div className="flex flex-col gap-4 pt-1">
            {/* 2. Seasonal Theme Selector */}
            <div className="flex flex-col gap-2">
              <label className="text-xs font-semibold text-slate-300 flex items-center gap-1.5">
                <Palette className="w-3.5 h-3.5 text-purple-400" />
                <span>季節主題 (Seasonal Themes)</span>
              </label>
                ...
                {themeList.map(([key, config]) => (
                  <button
                    ...
                    className={`flex items-center justify-between p-2 rounded-xl text-xs transition border text-left ${
                      settings.theme === key
                        ? 'bg-pink-950/60 border-pink-500/50 text-white shadow-sm ring-1 ring-pink-500/40'
                  ...
                  >
                    ...
                    <div
                      className="w-2.5 h-2.5 rounded-full shrink-0 border border-white/20 ml-1"
                      style={{ backgroundColor: config.foliagePrimary }}
                    />
                  </button>
                ))}
              </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                ...
                <span className={`w-2 h-2 rounded-full ${settings.autoRotate ? 'bg-pink-400' : 'bg-slate-600'}`} />
              </button>
